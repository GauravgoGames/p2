...
export class MemStorage implements IStorage {
  ...
  async calculatePoints(matchId: number): Promise<void> {
    ...
    for (const prediction of matchPredictions) {
      ...
      // Always award toss prediction points if the prediction is correct
      if (match.tossWinnerId && prediction.predictedTossWinnerId === match.tossWinnerId) {
        pointsEarned += 1;
        reasons.push("Correct toss prediction");
      }

      // Match winner points only for matches with clear winners (not tie or void)
      if (match.status === 'completed' && match.status !== 'tie' && match.status !== 'void' && match.matchWinnerId && prediction.predictedMatchWinnerId === match.matchWinnerId) {
        pointsEarned += 1;
