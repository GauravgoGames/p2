...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/predictions", isAuthenticated, async (req, res) => {
    try {
      ...
      const { matchId, predictedTossWinnerId, predictedMatchWinnerId } = req.body;
      ...
      await db.insert(predictions).values({
        matchId,
        userId: userId,
        predictedTossWinnerId,
        predictedMatchWinnerId
      });

      // Get updated prediction counts
      const tossCounts = await db
        .select({
          teamId: predictions.predictedTossWinnerId,
          count: sql`count(*)::int`,
        })
        .from(predictions)
        .where(eq(predictions.matchId, matchId))
        .groupBy(predictions.predictedTossWinnerId);

      const matchCounts = await db
        .select({
          teamId: predictions.predictedMatchWinnerId,
          count: sql`count(*)::int`,
        })
        .from(predictions)
        .where(eq(predictions.matchId, matchId))
        .groupBy(predictions.predictedMatchWinnerId);

      return {
        message: 'Prediction added successfully',
        tossPredictionCounts: Object.fromEntries(tossCounts.map(t => [t.teamId, t.count])),
        matchPredictionCounts: Object.fromEntries(matchCounts.map(t => [t.teamId, t.count]))
      };
    } catch (error) {
