...
export interface IStorage {
  ...
  //Prediction Stats
  getPredictionStats(matchId: number): Promise<{
        tossTeam1Count: number;
        tossTeam2Count: number;
        matchTeam1Count: number;
        matchTeam2Count: number;
    }>;
}
...
export class MemStorage implements IStorage {
  ...
  async getPredictionStats(matchId: number): Promise<{
        tossTeam1Count: number;
        tossTeam2Count: number;
        matchTeam1Count: number;
        matchTeam2Count: number;
    ...
    return {
      tossTeam1Count: predictions.filter(p => p.predictedTossWinnerId === match?.team1?.id).length,
      tossTeam2Count: predictions.filter(p => p.predictedTossWinnerId === match?.team2?.id).length,
      matchTeam1Count: predictions.filter(p => p.predictedMatchWinnerId === match?.team1?.id).length,
      matchTeam2Count: predictions.filter(p => p.predictedMatchWinnerId === match?.team2?.id).length,
    };
  }
